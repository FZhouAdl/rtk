...
* **cortex:** add Snowflake Cortex Code CLI (`cortex`) filter support — compacts `--version` to version number, compacts `mcp list` JSON to `status name` lines, passes through interactive sessions and all other commands with usage tracking
